...
const docsifyFunnyPlugin = (hook: DocsifyHooks, vm: any) => {
  ...
  function handleLottie(doc: Document) {
    ...
    const contentList = doc.querySelectorAll(
      `img[data-origin^='${fileType}>>']`
    );
    contentList.forEach((contentElement) => {
      ...
      const filePath =
        contentElement.getAttribute("src")?.replace(fileType + ">>", "") || "";
  ...
  function handleVideo(doc: Document) {
    const contentList = doc.querySelectorAll(
      `img[data-origin^='Bilibili>>'], img[data-origin^='Youtube>>']`
    );
    contentList.forEach((contentElement) => {
      const div = doc.createElement("div");
  ...
  function handleTable(doc: Document) {
    doc.querySelectorAll('pre[data-lang="table"]').forEach((item) => {
      const div = doc.createElement("div");
      createApp(Table, {
        markdown: item.querySelector("code")?.innerText,
      }).mount(div);
      item.replaceWith(div);
    });
  }

  function handleDemoPreview(doc: Document) {
    const obj = {};
    doc.querySelectorAll('pre[data-lang="demo"]').forEach((item) => {
      const div = doc.createElement("div");
      const id = nanoid();
      item.setAttribute("id", id);
      createApp(DemoPreview, {
        json5: item.querySelector("code")?.innerText,
        id: id,
      }).mount(div);
    });
  }
